...
fn validate_single_pattern(
    ...
    // Relationship or path pattern OR file-level pattern with multiple elements
        ...
        let is_path = pattern.elements[0].elements.len() == 2 || pattern.elements[1].elements.len() == 2;
        ...
        if is_file_level && !expected_sexp.contains("relationship_pattern") && !expected_sexp.contains("subject_pattern") {
