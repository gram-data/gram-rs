...
/// Convert a Value to JSON using mixed serialization strategy
///
/// # Strategy
///
/// **Native JSON** (simple, common types):
/// - VBoolean, VString, VArray, VMap
///
/// **Tagged objects** (complex, need disambiguation):
/// - VSymbol, VInteger, VDecimal, VRange, VMeasurement, VTaggedString
///
/// This optimizes for the common case (80% are simple types) while
/// preserving semantic information where it matters.
...
mod tests {
    ...
    fn test_from_pattern_with_properties() {
        ...
        // Check tagged JSON for integer
        let age_value = ast.subject.properties.get("age").unwrap();
        assert_eq!(age_value["type"], "Integer");
        assert_eq!(age_value["value"], 30);
    }
    ...
    fn test_value_serialization_simple_types() {
        ...
        // Array
        let v = value_to_json(&Value::VArray(vec![
            Value::VInteger(1),
            Value::VInteger(2),
        ]));
        assert!(v.is_array());
    ...
    fn test_value_serialization_map() {
        ...
        assert_eq!(v["key2"]["type"], "Integer");
        assert_eq!(v["key2"]["value"], 42);
    }
}
