...
fn test_element_counts_no_trailing_zeros() {
    ...
    assert_eq!(analysis.element_counts, vec![], "Atomic pattern should have empty element_counts");
    ...
    assert_eq!(analysis.element_counts, vec![3], "2-level tree should have [3], not [3, 0]");
    
    // 3-level tree: root -> node -> leaf
    // Should have element_counts = [1], not [1, 0, 0]
    let nested = Pattern::pattern("root".to_string(), vec![
        Pattern::pattern("node".to_string(), vec![
            Pattern::point("leaf".to_string()),
        ]),
    ]);
    let analysis = nested.analyze_structure();
    assert_eq!(analysis.element_counts, vec![1], "3-level linear tree should have [1], not [1, 0, 0]");
}
...
fn test_balanced_pattern_2_levels() {
    ...
    let child1 = Pattern::pattern("child1".to_string(), vec![
        Pattern::point("leaf1".to_string()),
        Pattern::point("leaf2".to_string()),
    ]);
    let child2 = Pattern::pattern("child2".to_string(), vec![
    ...
    assert!(analysis.nesting_patterns.contains(&"balanced".to_string()),
