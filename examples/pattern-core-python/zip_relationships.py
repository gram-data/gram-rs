...
def example_zip3_subjects():
    ...
    # Create PatternSubject nodes
    people = [
        pattern_core.PatternSubject.point(alice),
        pattern_core.PatternSubject.point(bob),
    ]
    
    companies = [
        pattern_core.PatternSubject.point(techcorp),
        pattern_core.PatternSubject.point(startup),
    ]
    ...
    # Note: zip3 works with Pattern, not PatternSubject, so we convert
    people_patterns = [pattern_core.Pattern.point(p.get_value().identity) for p in people]
    company_patterns = [pattern_core.Pattern.point(c.get_value().identity) for c in companies]
